...
export function isNumberValidator() {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    // if the control value is empty (empty string or null), let's return null (no error)
    // (every validator uses the same condition to correctly work together with the "required" validator)
