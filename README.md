...
Each exercise has the main focus eg **backend communication**, **forms** or **routing** but will touch on most of the other topics as well
...
- every exercise folder should be installed using `pnpm ci`
- every exercise can be started using `pnpm start` to run the app
- every exercise can start tests in watch mode using `pnpm run test:watch` (jest) or you can also start the test directly from your IDE.
...
- run `pnpm run test:watch` which will start tests in watch mode, so they will re-run any time you make changes to your code
