...
export class ProductListComponent {
  ...
  // TODO 8: let's start and streamify query signal using the toObservable() function
  // and put it as a first statement in the toSignal() function and attach it's own pipe() method
  ...
  // TODO 10: then we're going to use switchMap() operator which receives the value of query signal
  // and returns the productApiService.find() method with the query as an argument
  // in the ProductApiService, let's adjust the find() method to accept the "query" as a string
  // and use it to create object by instantiating Angular HttpParams class
  // use .set() method and set the "q" as a key and the query as a value
  // the object is then used as a "params" property of the options object
  // which is passed as the 2nd argument in the get() method
  // the running app should now support server side filtering!

  // TODO 11: let's move the tap from the nested pipe to the outer pipe at the end of the chain
  // in the nested pipe, lets add a new catchError() operator to handle the error
  // in the operator, we want to set error signal to string representation of the error
  ...
  removeProduct(productId: string) {
    ...
    // - a lot of logic in components and components are hard to test
    // - the logic itself is complex and involves many concepts to achieve the goal
    // - state in component (eg with sub routes) because they are not directly related (cant use input/output)
    ...
    // let's add a new add a new boolean signal based flag called loading (again) and initialize it to false
    ...
    // let's solve that by adding a condition in the first tap operator of the products signal
